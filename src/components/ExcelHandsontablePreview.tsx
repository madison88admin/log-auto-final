...
interface ExcelHandsontablePreviewProps {
  ...
  title?: string;
  onTablesExtracted?: (tables: { table: any[][], modelName: string }[]) => void;
...
export const ExcelHandsontablePreview: React.FC<ExcelHandsontablePreviewProps> = ({ excelFile, title, onTablesExtracted, onSelectedTableChange }) => {
  const [tables, setTables] = useState<{ table: any[][], modelName: string }[]>([]);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const foundTables: { table: any[][], modelName: string }[] = [];
        for (let i = 0; i < allRows.length; i++) {
          ...
          if (fuzzyHeaderMatch(row, EXPECTED_HEADER)) {
            ...
            foundTables.push({ table: paddedTable, modelName });
